...
!developmentChains.includes(network.name)
    ...
    : describe("Mutuals Unit Tests", async function () {
          ...
          beforeEach(async () => {
              ...
              await mutuals.setRewardRate("100")
          ...
          describe("setRewardRate", () => {
              it("Only owner could set new rate that is accurately updated", async () => {
                  const newRate = "1500"
                  await expect(mutuals.connect(player1).setRewardRate(newRate)).to.be.revertedWith(
                      "Ownable: caller is not the owner"
                  )
                  await mutuals.setRewardRate(newRate)
                  rate = await mutuals.rewardRate()
          ...
          describe("swap", () => {
              it("Outputs bought token and changes reserves pair in the pool", async () => {
                  ...
                  amountOut = parseInt(
                      ethers.utils.formatEther(BigInt(reserve1Before) - BigInt(reserve1After))
                  )
                  ...
                  await expect(
                      mutuals
                          .connect(player2)
                          .addLiquidity(amount0, (dy * BigInt(1053)) / BigInt(1000))
          ...
          describe("removeLiquidity", () => {
              it("Returns tokens to staker proportionate to the shares in current reserves balance", async () => {
                  shareToRemove = await mutuals.balanceOf(player1.address)
                  console.log(shareToRemove.toString())
                  await mutuals.connect(player1).removeLiquidity(BigInt(shareToRemove))
                  mockWETHBalance = parseInt(
                      ethers.utils.formatEther(await mockWETH.balanceOf(player1.address))
                  )
                  expectedmockWETHBalance = "100000000000000000000"
